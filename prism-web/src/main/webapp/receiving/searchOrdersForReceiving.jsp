...
<html>
    <head>
        <title>Search Orders for Receiving</title>
        <script type="text/javascript">
            ...
            function openOrder(orderId) {
                window.location = '<%=request.getContextPath()%>/viewReceiveOrder.do?selectedOrderId=' + orderId;
            }
        </script>
    </head>
    <body>
        <nested:form action="/receiving/searchOrdersForReceiving" method="get">
            ...
            <html:errors/>
            <p class="text-center"><strong><c:out value="${param.message}"/></strong></p>
            <fieldset>
                    ...
                    <div class="col-md-4">
                        <table>
                            ...
                            <tr>
                                ...
                                <td>
                                    <nested:select property="searchCriteria.shippingAddressId" style="width:210px" styleClass="chosen-select">
                                        <option value=""></option>
                                        <nested:optionsCollection name="lists" property="mailingAddresses" label="cityAndAddress" value="mailingAddressId"/>
                                    </nested:select>
                                </td>
                            </tr>
                        </table>
        ...
        <ul class="pagination">
            <c:if test="${fn:length(requestScope.result.pageList)!=0}">
                <c:if test="${requestScope.result.pageNo!=0}">
                    <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=0;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/receiving/searchOrdersForReceiving.do');">First</a></li>
                </c:if>
                <c:forEach begin="${requestScope.result.firstLinkedPage}" end="${requestScope.result.lastLinkedPage}" var="i">
                    <c:choose>
                        <c:when test="${requestScope.result.pageNo!=i}">
                            <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=<c:out value='${i}'/>;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/receiving/searchOrdersForReceiving.do');"><c:out value="${i+1}"/></a></li>
                        </c:when>
                        <c:otherwise>
            ...
            <c:if test="${requestScope.result.pageNo<requestScope.result.pageCount-1}">
                <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=<c:out value='${requestScope.result.pageCount}'/>;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/receiving/searchOrdersForReceiving.do');">Last</a></li>
            </c:if>
        </ul>
        Total results: <c:out value="${requestScope.result.nrOfElements}"/>
        <display:table name="requestScope.result.pageList" class="table table-bordered table-striped" export="false" id="parent">
            ...
            <c:choose>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByPoNo && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                    <display:column title='<a href="javascript:orderByPoNo();">PO#</a><img src="/images/displaytag/arrow_down.png" title="Sorted By PO# Descending">' property="purchaseOrderNumber"/>
                </c:when>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByPoNo && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='asc'}">
                    <display:column title='<a href="javascript:orderByPoNo();">PO#</a><img src="/images/displaytag/arrow_up.png" title="Sorted By PO# Ascending">' property="purchaseOrderNumber"/>
                </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByVendorName && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                    <display:column title='<a href="javascript:orderByVendorName();">Vendor</a><img src="/images/displaytag/arrow_down.png" title="Sorted By Vendor Name Descending">' property="vendor.externalOrgDetail.orgName"/>
                </c:when>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByVendorName && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='asc'}">
                    <display:column title='<a href="javascript:orderByVendorName();">Vendor</a><img src="/images/displaytag/arrow_up.png" title="Sorted By Vendor Name Ascending">' property="vendor.externalOrgDetail.orgName"/>
                </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByBuyerLastName && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                    <display:column
                            title='<a href="javascript:orderByBuyerLastName();">Buyer</a><img src="/images/displaytag/arrow_down.png" title="Sorted By Buyer Name Ascending">'
                ...
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByBuyerLastName && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='asc'}">
                    <display:column
                            title='<a href="javascript:orderByBuyerLastName();">Buyer</a><img src="/images/displaytag/arrow_up.png" title="Sorted By Buyer Name Ascending">'
                            property="purchaser.firstAndLastName"/>
                </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByOrderId && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                    <display:column
                            title='<a href="javascript:orderByOprNo();">Date Created</a><img src="/images/displaytag/arrow_down.png" title="Sorted By Date Created Descending">'
                            property="insertionDate" format="{0,date,MM/dd/yyyy}"/>
                </c:when>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByOrderId && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='asc'}">
                    <display:column
                            title='<a href="javascript:orderByOprNo();">Date Created</a><img src="/images/displaytag/arrow_up.png" title="Sorted By Date Created Ascending">'
                            property="insertionDate" format="{0,date,MM/dd/yyyy}"/>
                </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortBySuspenseDate && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                    <display:column
                            title='<a href="javascript:orderBySuspenseDate();">Suspense Date</a><img src="/images/displaytag/arrow_down.png" title="Sorted By Suspense Date Descending">'
                ...
                <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortBySuspenseDate && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='asc'}">
                    <display:column
                            title='<a href="javascript:orderBySuspenseDate();">Suspense Date</a><img src="/images/displaytag/arrow_up.png" title="Sorted By Suspense Date Ascending">'
                            property="suspenseDate" format="{0,date,MM/dd/yyyy}"/>
                </c:when>
                <c:otherwise>
            ...
            <display:column title="Delivery Location" property="shipToAddress.cityAndAddress"/>
        </display:table>
    </body>
</html>