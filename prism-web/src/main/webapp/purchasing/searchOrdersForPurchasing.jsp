...
<head>
    ...
    <script type="text/javascript">
        ...
        function openOrder(orderId) {
            window.location = '<%=request.getContextPath()%>/viewEditOrder.do?orderId=' + orderId;
        }
    </script>
</head>
<body>
<nested:form action="/purchasing/searchOrdersForPurchasing" method="get">
...
<ul class="pagination">
    <c:if test="${fn:length(requestScope.result.pageList)!=0}">
        <c:if test="${requestScope.result.pageNo!=0}">
            <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=0;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/purchasing/searchOrdersForPurchasing.do');">First</a></li>
        </c:if>
        <c:forEach begin="${requestScope.result.firstLinkedPage}" end="${requestScope.result.lastLinkedPage}" var="i">
            <c:choose>
                <c:when test="${requestScope.result.pageNo!=i}">
                    <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=<c:out value='${i}'/>;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/purchasing/searchOrdersForPurchasing.do');"><c:out value="${i+1}"/></a></li>
                </c:when>
                <c:otherwise>
    ...
    <c:if test="${requestScope.result.pageNo<requestScope.result.pageCount-1}">
        <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=<c:out value='${requestScope.result.pageCount}'/>;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/purchasing/searchOrdersForPurchasing.do');">Last</a></li>
    </c:if>
</ul>
