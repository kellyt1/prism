...
<html>
    <head>
        <title>Search Orders for Finance</title>
        <script type="text/javascript">
            ...
            function openOrder(orderId) {
                window.location = '<%=request.getContextPath()%>/viewOrderReadOnly.do?orderId=' + orderId;
            }
        </script>
    </head>
    <body>
        <nested:form action="/finance/searchOrdersForFinance" method="get" styleId="searchCriteriaForm">
        ...
        <ul class="pagination">
            <c:if test="${fn:length(requestScope.result.pageList)!=0}">
                <c:if test="${requestScope.result.pageNo!=0}">
                    <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=0;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/finance/searchOrdersForFinance.do');">First</a></li>
                </c:if>
                <c:forEach begin="${requestScope.result.firstLinkedPage}" end="${requestScope.result.lastLinkedPage}" var="i">
                    <c:choose>
                        <c:when test="${requestScope.result.pageNo!=i}">
                            <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=<c:out value='${i}'/>;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/finance/searchOrdersForFinance.do');"><c:out value="${i+1}"/></a></li>
                        </c:when>
                        <c:otherwise>
            ...
            <c:if test="${requestScope.result.pageNo<requestScope.result.pageCount-1}">
                <li><a href="javascript:document.forms['searchOrdersForm2'].elements['searchCriteria.pageNo'].value=<c:out value='${requestScope.result.pageCount}'/>;setActionAndSubmit('searchOrdersForm2','<%=request.getContextPath()%>/finance/searchOrdersForFinance.do');">Last</a></li>
            </c:if>
        </ul>
        ...
        <c:choose>
            <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByOrderId && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                <display:column
                        title='<a href="javascript:orderByOprNo();">OPR#</a><img src="/images/displaytag/arrow_down.png" title="Sorted By OPR# Descending">' class="text-center">
        ...
        <c:choose>
            <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByPoNo && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                <display:column
                        title='<a href="javascript:orderByPoNo();">PO#</a><img src="/images/displaytag/arrow_down.png" title="Sorted By PO# Descending">'
            ...
            <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByPoNo && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='asc'}">
                <display:column
                        title='<a href="javascript:orderByPoNo();">PO#</a><img src="/images/displaytag/arrow_up.png" title="Sorted By PO# Ascending">'
                        property="purchaseOrderNumber"/>
            </c:when>
            <c:otherwise>
                <display:column title="<a href='orderByPoNo();'>PO#</a>" property="purchaseOrderNumber"/>
            </c:otherwise>
        </c:choose>
        <c:choose>
            <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByVendorName && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='desc'}">
                <display:column
                        title='<a href="javascript:orderByVendorName();">Vendor</a><img src="/images/displaytag/arrow_down.png" title="Sorted By Vendor Name Descending">'
            ...
            <c:when test="${requestScope.searchOrdersForm2.searchCriteria.sortBy==sortByVendorName && requestScope.searchOrdersForm2.searchCriteria.sortMethod=='asc'}">
                <display:column
                        title='<a href="javascript:orderByVendorName();">Vendor</a><img src="/images/displaytag/arrow_up.png" title="Sorted By Vendor Name Ascending">'
                        property="vendor.externalOrgDetail.orgName"/>
            </c:when>
            <c:otherwise>
